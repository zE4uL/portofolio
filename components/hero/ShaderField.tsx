...
const FRAG = /* glsl */ `
  ...
  void main() {
    ...
    vec3 color = mix(uColorB, uColorA, n * 0.85);

    // ── Edge vignette — only the very outer rim fades; centre stays bright ──
    float vignette = smoothstep(0.0, 0.3,
  ...
  if (reducedMotion) {
    return (
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse 80% 60% at 50% 40%, rgba(124,92,255,0.28) 0%, rgba(10,10,11,0) 70%)",
        }}
      />
    );
  }
